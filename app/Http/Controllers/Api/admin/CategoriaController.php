...
class CategoriaController extends Controller
{
    ...
    {
        $data = Categoria::orderBy("orden")->get(["id","nombre"]); // de esta forma unicamente se llaman los campos necesitados para visualizar
    ...
    public function store(Request $request)
    {
        ...
        {
            $img = $request->urlfoto;
            $folderpath = "/img/categoria/";//se guarda la imagen
    ...
    public function destroy($id)
    {
        $data = Categoria::find($id);
        $data->delete();
        return response()->json('Categoria Borrada', 201);
    }
}
